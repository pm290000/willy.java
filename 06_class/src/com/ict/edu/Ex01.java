...
//	2) 상수 : 한번 지정되면 변경할 수 없는 데이터를 저장하는 공간
//				모든 글자가 대문자, 단어와 단어 사이에 (언더스코어)를 사용할 수 있다.

// 3. 멤버메소드 : 동작, 기능, 작동 하는 것
//	- 해당 메소드를 호출(실행)하면 해당 메소드 등 안에 존재하는 내용이 실행된다.
//	- 메소드는 다른 메소드를 호출할 수 있다.
//	- 해당 메소드를 호출해서 내용이 끝나면 메소드를 호출한 곳으로 되돌아 간다.
//	1) 메소드 구성 : [접근제한자] [메소드종류] 변환형 메소드이름([인자=매개변수]){실행내용}
...
//		라) 메소드이름 : 첫글자가 소문자 나머지 소문자, 두단어 이상일 때 단어 첫 글자 대문자
...
//	4. static 과 instance : 맴버메소드와 멤버 필드에 공통으로 해당 되는 사항
//	1) instance(인스턴스) : 객체 생성
//		- 인스턴스 필드, 인스턴스 메소드 => 객체가 생성될 때 같이 생성되는 필드와 메소드
//		- 일반적인 클래스에 맴버메소드나 멤버 필드는 모두 인스턴스이다.
//		- 호출방법 : 객체참조변수.멤버필드, 객체참수변수.멤버메소드
//		- heap 메모리에 만들어진다.	
	
//	2) static
//		- 객체 생성과 상관없이 미리 만들어진 멤버필드와 멤버메소드
//		- static은 반드시 메소드나 필드에 static 이라고 표시를 해야 한다.
//		- 언제든지 호출해서 사용 가능
//		- static 메모리에 만들어진다.
...
//		6) 생성자도 오버로딩이 가능하다. 즉 하나의 클래스 안에 여러개의 생성자가 존재 할 수 있다.
//		
